import { GameRecord } from "./gameRecord";
import mysql from "mysql2/promise";

export class GameGateway {
  async findLatest(conn: mysql.Connection): Promise<GameRecord | undefined> {
    const gameSelectResult = await conn.execute<mysql.RowDataPacket[]>(
      "select id, started_at from games order by id desc limit 1",
    );

    const record = gameSelectResult[0][0];

    if (!record) {
      return undefined;
    }

    return new GameRecord(record["id"], record["stated_at"]);
  }

  async insert(conn: mysql.Connection, statedAt: Date): Promise<GameRecord> {
    const gameInsertResult = await conn.execute<mysql.ResultSetHeader>(
      "insert into games (started_at) values (?)",
      [statedAt],
    );

    const gameId = gameInsertResult[0].insertId;

    return new GameRecord(gameId, statedAt);
  }
}
